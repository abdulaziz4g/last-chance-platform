...
export function Card({
  ...
  return (
    <div
      className={`rounded-2xl border border-zinc-200 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)] dark:border-white/[0.06] dark:bg-ink-900 ${className}`}
    >
      {children}
    </div>
  );
    ...
    <Card className="px-5 py-4">
      ...
      <p
        className={`tnum mt-1.5 text-2xl font-semibold ${
          accent ? 'text-brass-500 dark:text-brass-300' : ''
        }`}
      >
        {value}
      </p>
        ...
        <thead>
          <tr className="border-b border-zinc-200 text-left dark:border-white/[0.06]">
            {head.map((h) => (
              <th
                key={h}
                className="px-4 py-3 text-[11px] font-medium uppercase tracking-[0.12em] text-zinc-500 dark:text-zinc-400"
              >
                {h}
              </th>
            ))}
...
const CHIP_STYLES: Record<string, string> = {
  ...
  DRAFT: 'bg-zinc-500/10 text-zinc-500 ring-zinc-500/20',
  ...
  INITIATED: 'bg-zinc-500/10 text-zinc-500 ring-zinc-500/20',
  ...
  SKIPPED: 'bg-zinc-500/10 text-zinc-500 ring-zinc-500/20',
...
export function StatusChip({ status }: { status: string }) {
  const style = CHIP_STYLES[status] ?? 'bg-zinc-500/10 text-zinc-500 ring-zinc-500/20';
  return (
    <span
